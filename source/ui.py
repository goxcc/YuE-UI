...
def get_models(model_dir):
    ...
    for model in models:
        ...
        # model_path = os.path.join(model_dir, lower_name, "transformers/default/1", lower_name)
        if 's1' in lower_name:
            model_path = os.path.join(model_dir, lower_name, "transformers/default/1", 'YuE-s1-7B-anneal-en-cot-exl2')
            stage1_models.append(model_path)
        if 's2' in lower_name:
            model_path = os.path.join(model_dir, lower_name, "transformers/default/1", 'YuE-s2-1B-general-exl2')
